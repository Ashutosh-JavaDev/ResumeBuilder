...
public class Firstpage extends JFrame {
    ...
    public Firstpage() {
        setTitle("Custom Panel Example");
        setSize(1000, 600);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setLayout(null);

        // Initialize and add the first panel
        newPanel();

        // Initialize and add the second panel to the first panel
        SecondPanel();

        setVisible(true);
    }

    // Method to create and add the first panel
    public void newPanel() {
        firstPanel = new JPanel() {
            @Override
            protected void paintComponent(Graphics g) {
        ...
        firstPanel.setSize(990, 590);
        firstPanel.setLayout(null); // Changed to null layout for custom positioning
        firstPanel.setBounds(5, 5, 990, 590);
        firstPanel.setOpaque(false);
        add(firstPanel);
    }

    // Method to create and add the second panel to the first panel
    public void SecondPanel() {
        secondPanel = new JPanel() {
            @Override
            protected void paintComponent(Graphics g) {
        ...
        secondPanel.setSize(440, 290);
        secondPanel.setLayout(null); // Optional: Choose a layout that suits your needs
        secondPanel.setBounds(5, 5, 440, 290);
        secondPanel.setOpaque(false);

        firstPanel.add(secondPanel);
        firstPanel.revalidate(); // Revalidate to recalculate the layout
        firstPanel.repaint();    // Repaint to refresh the display
    }
